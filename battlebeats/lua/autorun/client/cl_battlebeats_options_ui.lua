local function CreateCustomCheckbox(parent, x, y, labelText, cvarName, helpText)
    ...
    box.Paint = function(self, w, h)
        draw.RoundedBox(4, 0, 0, w, h, GetConVar(cvarName):GetBool() and Color(50, 255, 50) or Color(90, 90, 90))
    end
    box.OnMousePressed = function()
        RunConsoleCommand(cvarName, GetConVar(cvarName):GetBool() and "0" or "1")
    end

    if helpText then
        box:SetTooltip(helpText)
    ...
    local spacing = 8
    local totalWidth = checkboxSize + spacing + label:GetWide()
    panel:SetWide(totalWidth)
    panel:SetPos(x - totalWidth / 2, y)

    box:SetPos(0, (panel:GetTall() - checkboxSize) / 2)
    label:SetPos(checkboxSize + spacing, (panel:GetTall() - label:GetTall()) / 2)
    ...
    box.OnCursorEntered = function(self)
        self:SetCursor("hand")
    end
    box.OnCursorExited = function(self)
        self:SetCursor("arrow")
    end
...
concommand.Add("battlebeats_options", function(ply, cmd, args)
    ...
    contentPanel:SetSize(frame:GetWide() - 20, frame:GetTall() - 90)
    contentPanel:SetPos(10, 80)
    ...
    for i, category in ipairs(categories) do
        ...
        if category.name == "Sound" then
            ...
            CreateCustomComboBox(panel, 90, 230, "On death behavior", "battlebeats_disable_mode",
                { "Nothing", "Mute completely", "Lower volume" })
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 140, "Enable Ambient", "battlebeats_enable_ambient")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 185, "Enable Combat", "battlebeats_enable_combat")
        elseif category.name == "Notifications" then
            ...
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 1.4, 10, "Notification always visible",
            "battlebeats_persistent_notification",
                "The notification will remain visible for the entire duration of the music, instead of disappearing after a few seconds")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 4, 50, "Show progress bar",
            "battlebeats_show_status_bar",
                "Displays a progress bar for the current track (showing total time and current time) at the bottom of the notification")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 1.4, 50, "Show replay notification",
            "battlebeats_show_notification_after_continue",
                "Displays a notification even if the same track is replayed (e.g. after leaving combat before the wait time ends). \n(This is enabled by default if 'Notification always visible' is active)")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 4, 90, "Skip Nombat track names",
            "battlebeats_skip_nombat_names", "Hides track names like C7 or A2 when playing tracks from Nombat packs")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 1.4, 90, "Show pack name in notification",
            "battlebeats_show_notification_pack_name", "Shows the pack name of the currently played track in the notification")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 130, "Enable preview notification",
            "battlebeats_show_preview_notification", "Shows notification when you are previewing tracks")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 170, "Show notification visualizer",
            "battlebeats_show_notification_visualizer", "Show FFT bars in the track notification")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 210, "Visualizer amplitude smoothing",
                "battlebeats_visualizer_smooth", "Enable amplitude smoothing for the visualizer")
            CreateArrowStepper(panel, (panel:GetWide() - 300) / 2, 250, "Visualizer Boost",
                "battlebeats_visualizer_boost", 1, 20,
                "Multiplier for visualizer amplitude boost (used in log scale)")
            CreateArrowStepper(panel, (panel:GetWide() - 300) / 2, 300, "Notification X position",
                "battlebeats_notif_x", 0, ScrW(), "Default: " .. tostring(ScrW() - 310))
            CreateArrowStepper(panel, (panel:GetWide() - 300) / 2, 350, "Notification Y position",
                "battlebeats_notif_y", 0, ScrH(), "Default: " .. tostring(ScrH() / 6))
        elseif category.name == "M. Player" then
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 10, "Switch to current pack only",
            "battlebeats_exclusive_play",
                "When switching between ambient and combat, only tracks from the same pack will be used. When a track ends naturally, the next one will still be chosen randomly from all enabled packs")
            CreateArrowStepper(panel, (panel:GetWide() - 300) / 2, 50, "Ambient wait time (in seconds)",
            "battlebeats_ambient_wait_time", 1, 120,
                "Wait time defines how long the music player will wait before replacing the previous track with a new one")
            CreateArrowStepper(panel, (panel:GetWide() - 300) / 2, 100, "Combat wait time (in seconds)",
            "battlebeats_combat_wait_time", 1, 120,
                "Wait time defines how long the music player will wait before replacing the previous track with a new one")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 150, "Always continue previous track",
            "battlebeats_always_continue",
                "Skips the wait time and always resumes the previous track until it finishes playing")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 190, "NPC's fight triggers combat",
                "battlebeats_npc_combat", "Triggers combat when an NPC is targeting other NPCs or players")
            CreateCustomComboBox(panel, 100, 230, "Continue Mode", "battlebeats_continue_mode",
                { "Resume from last position", "Play simultaneously" },
                "Resume from last position: the track will continue from where it left off before switching.\nPlay simultaneously: tracks continue playing in the background during switches (not actually audible - they just remain active in the background)")
        elseif category.name == "Misc" then
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 10, "NPC Combat requires LoS",
            "battlebeats_detection_mode",
                "LoS - Line of Sight. If enabled, combat will only trigger when you have visual contact with an enemy")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 50, "Auto Popup", "battlebeats_autopopup",
            "Automatically open the BattleBeats pack selector on startup if no packs are selected")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 90, "Load local packs", "battlebeats_load_local_packs",
            "Enables loading music packs directly from the addons/ folder without needing to upload them to Workshop. This will not work if Debug Mode is enabled\n(Not intended for testing sound packs - use debug mode for that instead)\n(Requires restart or packs reload)")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 130, "Debug Mode", "battlebeats_debug_mode",
            "Used for testing sound packs and debugging functions. Some features may be disabled while debug mode is active\n(Requires restart or packs reload)")
            CreateCustomCheckbox(panel, contentPanel:GetWide() / 2, 170, "Lower volume in menu", "battlebeats_lower_volume_in_menu", "Lowers volume in game/spawn menu ('escape' menu won't work in singleplayer - timers pause when menu is open)")
        end
